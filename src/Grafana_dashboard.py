...
class GrafanaDashboard(Base):
    ...
    def __init__(self):
        ...
        self.grafana_api = self.login(auth_type="credentials", target_server=False)

    def create_or_update(self, dashboard={'dashboard': {...}, 'folderId': 0, 'overwrite': True}, target_server=False):
        """
        :param dashboard:
        :return:
        ...
        print(GrafanaFace)
        try:
            # change api object to point to target server if import has to done on target server
            if target_server:
                self.grafana_api = self.login(auth_type="credentials", target_server=True)
    ...
    def import_dashboard(self, dashboard={'dashboard': {...}, 'folderId': 0, 'overwrite': True}, target_server=True):
        """
        ...
        try:
            imported_dashboard = self.create_or_update(dashboard=dashboard, target_server=self.cross_server_import)
        except Exception as e:
            print("Got error: %s" % e)
